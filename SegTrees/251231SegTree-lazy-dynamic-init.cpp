...
class SegTree {
    ...
    void push_up(int p, long long l, long long r) {
        long long mid = (l + r) >> 1;
        T le = (ls[p] == 0 ? policy.init(l, mid) : ds[ls[p]]);
        T re = (rs[p] == 0 ? policy.init(mid + 1, r) : ds[rs[p]]);
        ds[p] = policy.op(
            {le, mid - l + 1},
            {re, r - mid}
        );
    ...
    void push_down(int p, long long l, long long r) {
        long long mid = (l + r) >> 1;
        long long size = r - l + 1;
        if (!ls[p]) {
            ls[p] = create_node(policy.init(l, mid));
        }
        if (!rs[p]) {
            rs[p] = create_node(policy.init(mid + 1, r));
        }
    ...
    T query(int p, long long l, long long r, long long nl, long long nr) {
        if (!p) {
            return policy.init(std::max(l, nl), std::min(r, nr));
        }
        if (nl <= l && r <= nr) {
            return ds[p];
        }
